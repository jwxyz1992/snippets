...
class encn_Oxford {
    ...
    async displayName() {
        ...
        if (locale.indexOf('CN') != -1) return 'Bing词典';
        if (locale.indexOf('TW') != -1) return 'Bing词典';
        return 'Bing EN->CN Dictionary';
    }
    ...
    async findOxford(word) {
        ...
        let entries = doc.querySelectorAll('.def_row.df_div1');
        ...
        for (const entry of entries) {
            ...
            let chn_tran = T(entry.querySelector('.p1-1.b_regtxt'));
            chn_tran = `<span class='chn_tran'>${chn_tran}</span>`;
            definition += `${pos}<span class='tran'>${chn_tran}</span>`;

            let examps = doc.querySelectorAll('.se_li') || [];
            if (examps.length > 0 && this.maxexample > 0) {
                definition += '<ul class="sents">';
                for (const [index, examp] of examps.entries()) {
                    if (index > this.maxexample - 1) break; // to control only 2 example sentence.
                    let eng_examp = T(examp.querySelector('.sen_en.b_regtxt'));
                    let chn_examp = T(examp.querySelector('.sen_cn.b_regtxt'));
